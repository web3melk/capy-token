...
describe('Capy', function () {
  before(async function () {
    ...
    this.buy = async function (wallet, amountEth) {
      ...
      return await this.uniswapRouter
        .connect(wallet)
    ...
    this.deployUniswap = async function () {
      ...
      this.uniswapRouter = await uniswapRouter.deploy(
    ...
    this.emittedEvent = function (name, args, log = false) {
      return (
        this.receipt.events.find((event) => {
  ...
  describe("Deployment", function () {
    ...
    it("mints 1 billion tokens tokens", async function () {
      ...
      expect(await this.contract.balanceOf(this.contract.address)).to.equal(
  ...
  describe("receive", function () {
    describe("pool not created", function () {
      beforeEach(async function () {
        expect(await this.contract.uniswapV2Pair()).to.equal(
      ...
      it("when 0.5 eth should add OG and exclude from fee", async function () {
        expect(await this.contract.totalOGs()).to.equal(1)
        await expect(
          this.signers[3].sendTransaction({
            to: this.contract.address,
            value: eth(0.5),
          })
        )
          .to.emit(this.contract, "ExcludeFromFees")
          .withArgs(this.signers[3].address, true)
    ...
    describe("pool already created", function () {
      beforeEach(async function () {
        ...
        expect(await this.contract.uniswapV2Pair()).not.to.equal(
          "0x0000000000000000000000000000000000000000"
        )
      })
      it("should revert", async function () {
        await expect(
          this.signers[6].sendTransaction({
            to: this.contract.address,
            value: eth(0.5),
          })
  ...
  describe("toogleCheckReceive", function () {
    it("should block not owner", async function () {
      await expect(
        this.contract.connect(this.notAdmin).toogleCheckReceive(false)
  ...
  describe("setSwapEnabled", function () {
    it("should block not owner", async function () {
      await expect(this.contract.connect(this.notAdmin).setSwapEnabled(true)).to.be.revertedWith(
  ...
  describe("excludeFromFees", function () {
    it("should put and remove from list", async function () {
      expect(await this.contract.isExcludedFromFees("0x90F79bf6EB2c4f870365E785982E1f101E93b906"))
        .to.be.false
      await expect(
        this.contract.excludeFromFees("0x90F79bf6EB2c4f870365E785982E1f101E93b906", true)
      )
        .to.emit(this.contract, "ExcludeFromFees")
        .withArgs("0x90F79bf6EB2c4f870365E785982E1f101E93b906", true)
      expect(await this.contract.isExcludedFromFees("0x90F79bf6EB2c4f870365E785982E1f101E93b906"))
        .to.be.true
      await expect(
        this.contract.excludeFromFees("0x90F79bf6EB2c4f870365E785982E1f101E93b906", false)
      )
        .to.emit(this.contract, "ExcludeFromFees")
        .withArgs("0x90F79bf6EB2c4f870365E785982E1f101E93b906", false)
      expect(await this.contract.isExcludedFromFees("0x90F79bf6EB2c4f870365E785982E1f101E93b906"))
        .to.be.false
    })
  })
  ...
  describe("excludeFromMaxTransaction", function () {
    it("should put and remove from list", async function () {
      expect(
        await this.contract.isExcludedFromMaxTransaction(
          "0x90F79bf6EB2c4f870365E785982E1f101E93b906"
        )
      ).to.be.false
      await expect(
        this.contract.excludeFromMaxTransaction("0x90F79bf6EB2c4f870365E785982E1f101E93b906", true)
      )
        .to.emit(this.contract, "ExcludeFromLimits")
        .withArgs("0x90F79bf6EB2c4f870365E785982E1f101E93b906", true)
      expect(
        await this.contract.isExcludedFromMaxTransaction(
          "0x90F79bf6EB2c4f870365E785982E1f101E93b906"
      ...
      await expect(
        this.contract.excludeFromMaxTransaction("0x90F79bf6EB2c4f870365E785982E1f101E93b906", false)
      )
        .to.emit(this.contract, "ExcludeFromLimits")
        .withArgs("0x90F79bf6EB2c4f870365E785982E1f101E93b906", false)
      expect(
  ...
  describe("launch", function () {
    ...
    it("should create the pool and add liquidity", async function () {
      await this.addOGs(9)
      expect(await this.contract.balanceOf(this.contract.address)).to.equal(eth(1_000_000_000))
  ...
  describe("transfer", function () {
    describe("trading not started", function () {
      describe("withdrawTokens", function () {
        describe("without balance", function () {
          it("revert", async function () {
            ...
            await expect(this.contract.withdrawTokens()).to.be.revertedWith(
        ...
        describe("with balance", function () {
          describe("from deployer", function () {
            it("send tokens to OGs", async function () {
              ...
              await expect(this.contract.withdrawTokens())
                .to.emit(this.contract, "Transfer")
                .withArgs(this.contract.address, this.deployer.address, eth(1_000_000_000))
              expect(await this.contract.balanceOf(this.contract.address)).to.equal(eth(0))
              expect(await this.contract.balanceOf(this.deployer.address)).to.equal(
                eth(1_000_000_000)
              )
            })
          })
          describe("from OG", function () {
            it("should revert transaction", async function () {
              await expect(this.contract.connect(this.og).withdrawTokens()).to.be.revertedWith(
                "caller is not the owner or OG after launch"
              )
            })
          })
        })
      ...
      it("from deployer to contract", async function () {
        ...
        await expect(this.contract.transfer(this.contract.address, eth(500)))
          .to.emit(this.contract, "Transfer")
          .withArgs(this.deployer.address, this.contract.address, eth(500))
        expect(await this.contract.balanceOf(this.contract.address)).to.equal(eth(500))
        expect(await this.contract.balanceOf(this.deployer.address)).to.equal(
          previousDeployerBalance.sub(eth(500))
        )
      })
      it("from deployer to another wallet", async function () {
        await this.contract.withdrawTokens()
        let previousDeployerBalance = await this.contract.balanceOf(this.deployer.address)
        await expect(this.contract.transfer(this.signers[4].address, eth(500)))
          .to.emit(this.contract, "Transfer")
          .withArgs(this.deployer.address, this.signers[4].address, eth(500))
      ...
      it("from any address to another address", async function () {
        ...
        await expect(
          this.contract.connect(this.signers[4]).transfer(this.signers[5].address, eth(500))
        ).to.revertedWith("Trading not started")
      })
      it("from whitelisted address to another address", async function () {
        ...
        await expect(
          ...
          .to.emit(this.contract, "Transfer")
          .withArgs(this.signers[4].address, this.signers[5].address, eth(500))
      ...
      describe("fees", function () {
        describe("buy", function () {
          it("do not collect fees before 500 buys", async function () {
            ...
            expect(parseFloat(ethers.utils.formatEther(balance)) * this.tokenPrice).to.be.closeTo(
              3.6,
              0.02
            )
          })
        ...
        describe("sell", function () {
          beforeEach(async function () {
            ...
            wallet = wallet.connect(ethers.provider)
            await this.signers[15].sendTransaction({
              to: wallet.address,
          ...
          it("do not collect fees before 1000 buys", async function () {
            await this.contract
              ...
              .transfer(this.sellSigner.address, eth(1_000_000))
            expect(this.tokenPrice).to.be.closeTo(0.0000036, 0.0000001)
            ...
            let wouldReceivedWithoutFeeETH = ethFromDollar(1_000_000 * this.tokenPrice)
            expect(parseFloat(ethers.utils.formatEther(received))).to.be.closeTo(
              parseFloat(ethers.utils.formatEther(wouldReceivedWithoutFeeETH)),
              1000
            )
        ...
        describe("auto swap", function () {
          beforeEach(async function () {
            ...
            await this.contract
              .connect(this.signers[14])
              .approve(this.uniswapRouter.address, eth(1))
          })
    ...
    describe("trading started", function () {
        ...
        describe("with balance", function () {
          describe("from deployer", function () {
            it("send tokens to OGs from deployer", async function () {
              ...
              expect(await this.contract.balanceOf(this.og.address)).to.equal(
                eth("50000055.555555555555555555")
              )
            })
          })
          describe("from OG", function () {
            it("send tokens to OGs from OG", async function () {
              ...
              expect(await this.contract.balanceOf(this.contract.address)).to.equal(eth("500"))
              expect(await this.contract.totalOGs()).to.equal(9)
              await expect(this.contract.connect(this.og).withdrawTokens())
                .to.emit(this.contract, "Transfer")
                .withArgs(this.contract.address, this.og.address, eth("55.555555555555555555"))
            })
          })
        })
      })
      ...
      describe("fees", function () {
        ...
        describe("sell", function () {
          it("do collect 5% of fees before 1000 buys", async function () {
            ...
            await this.contract
              .connect(this.deployer)
              .transfer(this.signers[10].address, eth(1_000_000))
            expect(this.tokenPrice.toString()).to.equal("0.0000324")
            let previousBalance = await ethers.provider.getBalance(this.signers[10].address)
            await this.sell(this.signers[10], eth(1_000_000))
            ...
            let received = newBalance.sub(previousBalance)
            let wouldReceive = ethFromDollar(1_000_000 * (1 - sellFee) * this.tokenPrice)
            expect(parseFloat(ethers.utils.formatEther(received))).to.be.closeTo(
              parseFloat(ethers.utils.formatEther(wouldReceive)),
              1000
            )
            let wouldReceivedWithoutFeeETH = ethFromDollar(1_000_000 * this.tokenPrice)
      ...
      describe("OG wallet update on transfer", function () {
        ...
        it("removes when balance ends with less than distributed", async function () {
          await this.contract
            .connect(this.og)
            .transfer("0x2546BcD3c84621e976D8185a91A922aE77ECEc30", eth(800_000))
        ...
        it("updates when transfering all tokens", async function () {
          await this.contract
            .connect(this.og)
            .transfer("0x2546BcD3c84621e976D8185a91A922aE77ECEc30", eth(50000000))
          expect(await this.contract.balanceOf(this.og.address)).to.equal(eth(0))
          expect(await this.contract.isOG(this.og.address)).to.be.false
  ...
  describe("manualSwap", function () {
    ...
    describe("when OG", function () {
      beforeEach(async function () {
        ...
        await this.contract.transfer(this.contract.address, eth(100_000))
      });
      ...
      it('should swap tokens', async function () {
        expect(await this.contract.balanceOf(this.contract.address)).to.equal(eth(100_000))
        expect(await ethers.provider.getBalance(this.contract.address)).to.equal(eth(0));
        let previousOGBalance = await ethers.provider.getBalance(this.og.address);
        await this.contract.manualSwap();
